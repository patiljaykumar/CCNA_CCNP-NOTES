- **IGRP (Interior Gateway Routing Protocol)**
    ...
    - Metric - Distance Count (default limit- 100 | Max- 255)
    - Slow Convergence
...
- **AD- ==90 (internal), 170 (external), 5(summary route)==**
...
- **Metric- K values (Default uses K1(int Bandwidth not speed), K3 as these are static values) (****can be conf to use all 5 K values****)**
    - **K1= Bandwidth** (kbps)
    - **K2= Load**
    - **K3= Delay** (tens of microsec eg- 15= 150microsec=150x10<sup>\-6</sup> sec\] (sec= delay-valuex10<sup>\-5</sup>)
    - **K4= Reliability**
    - **K5= MTU**
- ==**Default Hop Count Limit- <ins>100</ins> | Command output Max- 255 | Practical Max- <ins>224</ins>**== 
...
- **<img src="../../_resources/5bfa56d49727a38c284a2029206ece51.png" alt="5bfa56d49727a38c284a2029206ece51.png" width="387" height="164" class="jop-noMdConv">**
...
1.  **Hello** (Opcode-5)
    ...
    - <img src="../../_resources/c98f9b1842e6ffbb6bed37b77118a6e8.png" alt="c98f9b1842e6ffbb6bed37b77118a6e8.png" width="260" height="324" class="jop-noMdConv">
...
3.  **Update** (Opcode-1)
    ...
    - <img src="../../_resources/6bf67eaf072957aaf54675cd5ca63a9c.png" alt="6bf67eaf072957aaf54675cd5ca63a9c.png" width="428" height="67" class="jop-noMdConv">
    - <img src="../../_resources/79a47a062f9fefb23514cfd84bb97c73.png" alt="79a47a062f9fefb23514cfd84bb97c73.png" width="240" height="129" class="jop-noMdConv">
    - <img src="../../_resources/59b1051408610381029209e65040b23e.png" alt="59b1051408610381029209e65040b23e.png" width="321" height="229" class="jop-noMdConv">
4.  **Query** (Opcode-3)
...
5.  **Reply** (Opcode-4)
    ...
    - <ins>always unicast</ins>
6.  **SIA (Stuck in Active) Query**\- <ins>unicast</ins> must reply packets (If Router receives SIA Query it has to reply with SIA Reply, irrespective of it has info or not)
7.  **SIA Reply**\- <ins>unicast</ins> (If Router receives SIA Reply in response to SIA Query, it will reset active timer(180s))
...
- SIA Query & Reply are reliable messages used to prevent breaking of neighbourships (==very rare==)
    
    - eg- <img src="../../_resources/0a9f9c5605ab0fe02f8eb25e57e81d73.png" alt="0a9f9c5605ab0fe02f8eb25e57e81d73.png" width="424" height="108" class="jop-noMdConv">
    - If 1.1.1.1/24 goes down. (check timer by- `sh ip eigrp topology`)
        - R1 will Query R2. R2 check its RT if not query R3. If R3's RT has route, R3 will reply R2, R2 will reply R1
        - Imagine due to net congestion Reply from R3 lost. If R2 didn't reply within 180s(Active timer), R1 will break neigh with R2.
        - To provide R2 more time SIA was intro. After 90s R1 will send SIA query to R2. R2 will send SIA reply to R1, R1 will reset timer. (It will repeat sending SIA query and reseting 3 times then break neigh if no response)
        - ![b8a017fc549cab0b4c8bcf7567c2eadb.png](../../_resources/b8a017fc549cab0b4c8bcf7567c2eadb.png)
...
- <img src="../../_resources/ebbf66470e1643b9782db435f90e5b98.png" alt="ebbf66470e1643b9782db435f90e5b98.png" width="381" height="89" class="jop-noMdConv">
...
- **Hold Time**\- how long router waits for neighbors hello msgs before declaring it as inactive. (default is 3 times hello)
    ...
    - ==<span style="color: rgb(224, 62, 45);">**eg- if R1 & R2 hello=5s | on R1 hold config= 500s -> R2 will wait for R1 hello for 500s (you will see hold=500s in R2)**</span>==
        - <img src="../../_resources/b4a9671731558323cdf86e502be07093.png" alt="b4a9671731558323cdf86e502be07093.png" width="682" height="94" class="jop-noMdConv">
        - <img src="../../_resources/5d791f0abecbd1d2d69d0f69a53e6ebb.png" alt="5d791f0abecbd1d2d69d0f69a53e6ebb.png" width="666" height="159" class="jop-noMdConv">

* * *

## 3 Tables

## 1\. **Neighbor Table**\- List of neighboring routers (directly connected EIGRP neigh)
...
- int down (no network &lt;int&gt;)(k1-k5= 255)
    ...
    - <img src="../../_resources/0f5fdf881fd1c1c6709da0a2824f17f1.png" alt="0f5fdf881fd1c1c6709da0a2824f17f1.png" width="304" height="267" class="jop-noMdConv">
- <img src="../../_resources/2daa5aa8f8078c08078534e401a7eb79.png" alt="2daa5aa8f8078c08078534e401a7eb79.png" width="708" height="80" class="jop-noMdConv"></img>
...
## 2\. **Topology Table**\- contains all learned routes, Successor & Feasible successors with metrics(<ins>FD/AD</ins> \[AD-advertised distance\])

- **Successor** - best route (found in topology & RT) {<ins>*lowest FD*</ins>} (not sure- learned routes)
...
- **EIGRP Metric Formula**
    ...
    - <img src="../../_resources/d38e33a97cdfd287af0add595f02f4d4.png" alt="d38e33a97cdfd287af0add595f02f4d4.png" width="575" height="71" class="jop-noMdConv">
    - By default K1=K3=1, K2=K4=K5=0
    - <ins>RFC 7868 states if K<sub>5</sub>\=0, then reliability quotient = 1</ins>
    - <ins><img src="../../_resources/216903931855515bd79a03fea42a4f6b.png" alt="216903931855515bd79a03fea42a4f6b.png" width="510" height="226" class="jop-noMdConv"></ins>
    - <ins><img src="../../_resources/c0a960094eb485456ef0b2922c7d05ae.png" alt="c0a960094eb485456ef0b2922c7d05ae.png" width="403" height="281" class="jop-noMdConv"></ins>
    - eg1- <img src="../../_resources/91c1184981b19a934e13e99424aed67b.png" alt="91c1184981b19a934e13e99424aed67b.png" width="285" height="148" class="jop-noMdConv">
        - R1 to 2.2.2.2- Metric = 256(10+501) = 256x511 = <ins>130816</ins>  (Min.bandwidth= 10<sup>6</sup>)
        - `do show ip route eigrp`\- check Metric
        - ![cc2deeebe4dd0aa3bc5024830417898c.png](../../_resources/cc2deeebe4dd0aa3bc5024830417898c.png)
    - eg2- <img src="../../_resources/3885abd188713744a59c57375a4c37c8.png" alt="3885abd188713744a59c57375a4c37c8.png" width="565" height="272" class="jop-noMdConv">
    ...
    - <img src="../../_resources/56f225183a216492ca9c9daa74d6cf55.png" alt="56f225183a216492ca9c9daa74d6cf55.png" width="659" height="63" class="jop-noMdConv"></img>
...
## 3\. **Routing Table**\- List of best actionable routes for forwarding (AD/FD \[AD-administrative distance\])
...
- eg- In above topology, if FS1= 120, FS2= 120, FS3= 220 | FD of S= 200 (so RT will have S, FS1, FS2 \[because they are below 200\])
    ...
    - <img src="../../_resources/8ea5483126bcf76bf3086376c0b1a339.png" alt="8ea5483126bcf76bf3086376c0b1a339.png" width="410" height="99" class="jop-noMdConv">
    - <img src="../../_resources/812ecb80b256b98c270bee390851c209.png" alt="812ecb80b256b98c270bee390851c209.png" width="242" height="161" class="jop-noMdConv">
...
- 2 types of summarization
    ...
    1.  **Auto Summary** - routes auto-summarized to classful boundaries
        ...
        - eg <img src="../../_resources/e064688a9cb22ffc9ceeadd25e4f8962.png" alt="e064688a9cb22ffc9ceeadd25e4f8962.png" width="421" height="149" class="jop-noMdConv">
            
            - before
                - <img src="../../_resources/62fa38d995aa0b31b98df9b47c3fb728.png" alt="62fa38d995aa0b31b98df9b47c3fb728.png" width="770" height="99" class="jop-noMdConv">
            - after
                - <img src="../../_resources/5fdc1fc80ce877e693b1dd5bab210ba9.png" alt="5fdc1fc80ce877e693b1dd5bab210ba9.png" width="785" height="35" class="jop-noMdConv">
...
- <img src="../../_resources/db083979b1323708e019d24345d487c8.png" alt="db083979b1323708e019d24345d487c8.png" width="418" height="103" class="jop-noMdConv">
- Methods
...
- **Advantages over Classic**
    ...
    - Disadv- needs Router with 15+ version `(do sh ver | inc Version)`
- **Divided in 3 sub-sections**
    - **Address-family-** config IPv4/IPv6/VRF, AS no., net advertisement
    - **Topology Base-** config- changing entire metric, redistribution
    - **af-interface-** int specific config
- config
    - `router eigrp <name>` - name can be diff on all routers
    - `address-family <ipv4/ipv6> autonomous-system <ASN>` - ASno. needs to match (==classic & named mode can form neigh by same ASno.==)
    - `network <net-id> <wild>`
...
### **HMAC-SHA-256 auth (**Hashed Message Authentication Code-Secure Hash  Algorithm-256**)**

- Named mode supports MD5 & HMAC-SHA (more secure) auth
- uses a shared secret key across network devices to create and verify a one-way message digest for each packet, ensuring packet integrity and authenticity
    - eg- Device A & B same secret-key "K123". Device A sends packet "hello" + digest(created by HMAC-SHA256+K123) | Device B uses same key(K123) to recompute digest - if matches packet authentic
...
- used to reduce no. of query msgs. when config router as stub, it tells neigh not to send any queries (use- hub-spoke topologies lost route at hub can trigger queries to all neigh | disAdv- remote branches routers slow WAN links)
    
- generally config on end/ edge routers (i.e. Branches)
    
- Config on Branch1 & Branch2
    
    - `router eigrp <ASN>`
    - `eigrp stub`
- <img src="../../_resources/cb05bdeedc14910325837d700eae8e64.png" alt="cb05bdeedc14910325837d700eae8e64.png" width="441" height="149" class="jop-noMdConv"></img>
    - eg- if branches are config with Stub- inform HQ don't send queries (if 1 route down on HQ1 1)HQ1 will only query HQ2, then 2)HQ1 send updates to branches(inform of route down), 3)Branches query only HQ2)
    - After Basic config
    - B1- `debug eigrp packets query update`
    - to see working HQ1- `int l0`, `sh`
- **Stub Levels**
    
    - connected- Do advertise connected routes `(sh run | sec eigrp)`
...
- <ins>**NOTE**\- By default connected & summary routes are advertised</ins>
...
- way to change metric of route by adding fixed value(offset)
- It is applied inbound, outbound dir (alternative- change Bandwidth, delay)
- config
...
- ### Benefits of EIGRP over OSPF
    
    - **Faster Convergence**\- precalculated backup(feasible successor) vs ospf- recalculation entire path with SPF
    - **Supports Unequal-Cost Load Balancing** (using variance command)
    - **EIGRP uses multiple Metrics** (choose more optimized paths)
    - **No Periodic-Full-Updates** (like OSPF max age of LSAs after 30min)
