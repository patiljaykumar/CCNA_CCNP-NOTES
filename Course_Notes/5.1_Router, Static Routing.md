- Router starting info (not sure)
    ...
    - <img src="../../_resources/2864e76ff1217d7c53535c42172ede6e.png" alt="2864e76ff1217d7c53535c42172ede6e.png" width="396" height="476" class="jop-noMdConv">
- **Router**
    ...
    - Router-to-Router comm
        1.  incoming L2 frames dest MAC, FCS is verified, then frame decapsulated
        2.  L3- Reads dest IP, looks RT, Decrements TTL(-1), Recalculates IP header checksum
        3.  Re-encapsulates packet to new frame (new L2 header/trailer), sends via int
    - <ins>**Memory, CPU**</ins>
        - <ins>CPU</ins>\- managing OS, making routing decisions, processing requests(DNS,DHCP)
        - <ins>RAM</ins>\- Volatile memory, stores running-config
        - <ins>NVRAM</ins>\- Non-Volatile/ writable memory, stores startup-config
        - <ins>Flash</ins>\- Non-Volatile memory, stores OS image(Cisco IOS)
        - <ins>ROM</ins>\- Non-Volatile memory, stores firmware for startup(POST), bootstrap program to find & load OS from flash, password recovery
- **path-** sequence of hops a packet takes through net
    
- **Route-** specific path to reach the destination (known way, can be multiple routes for same dest)
    
- **Routing-** process of selecting best path(route) to forward IP packets in any net (to reach from src to dest)\[based on- Prefix length, AD, Metric\]
    
- **Routing Table-** Collection of best active routes (best paths) `sh ip route`
    
    - `netstat -r`\- **![a0f3dd09ee6f4f4ba3b6745bb2dd804f.png](../../_resources/a0f3dd09ee6f4f4ba3b6745bb2dd804f.png)**
    - **RIB(Routing Information Base)**\- DB of all learned routes(static, conn, dynamic). Every protocol has its own RIB \[<ins>Control plane</ins> for route selection\] - `sh ip ospf database`, `sh ip eigrp topology`
        - <img src="../../_resources/f15fb5f6694bd9bbdeb52d955fe38cfe.png" alt="f15fb5f6694bd9bbdeb52d955fe38cfe.png" width="365" height="155" class="jop-noMdConv">
        - <img src="../../_resources/60e4599bd2fa546608a341eecaa3eb62.png" alt="60e4599bd2fa546608a341eecaa3eb62.png" width="335" height="159" class="jop-noMdConv">
    - ****FIB(Forwarding Information Base)-**** optimized version(subset) derived from RIB with best path used for forwarding. used by CEF,1 of 2 components of CEF(other is Adjacency table- next hop L2 info)
        - <ins>data plane</ins> packet forwarding, has next-hop & out-int. built using efficient DS(trees- so lookup faster) `sh ip cef`
        - ![0ce8ea4e97d6bbb832ef19f22a91206f.png](../../_resources/0ce8ea4e97d6bbb832ef19f22a91206f.png)
        - <img src="../../_resources/b33e34f4bd0c57996f930c43641887f1.png" alt="b33e34f4bd0c57996f930c43641887f1.png" width="262" height="135" class="jop-noMdConv">
...
- Best path choosing sequence
    ...
    2.  **AD (Administrative Distance)**
        ...
        - ![a7c29e5ec6ab5f1d268bfb53aaab4e11.png](../../_resources/a7c29e5ec6ab5f1d268bfb53aaab4e11.png)
    3.  **Metric**
        
        - when multiple paths available from same protocol metric of paths compared for best path. <ins>**prefers least Metric**</ins> (eg- OSPF cost 10 prefered over 20)
    4.  **Load Balance**
...
- <img src="../../_resources/b9dc8314240607d5cd3dae8cab0c90d6.png" alt="b9dc8314240607d5cd3dae8cab0c90d6.png" width="396" height="129" class="jop-noMdConv">
- **R1**
...
- `ip route 40.1.1.0 255.255.255.0 g0/0`
    ...
    - ![6b9fb73203f3fd10640ce86e366174f0.png](../../_resources/6b9fb73203f3fd10640ce86e366174f0.png)
    - `show arp` | `clear arp` or `int <int>`, `sh`, `no sh`\- clear router arp
        - <img src="../../_resources/91eda396082cd7718f97cbd1c854c5f4.png" alt="91eda396082cd7718f97cbd1c854c5f4.png" width="840" height="127" class="jop-noMdConv">
    - ping 30.1.1.1- 5/5
...
4.  **Floating static route**
    ...
    - **<img src="../../_resources/b1142b3ec0642efe2a6fbd38bc4714bc.png" alt="b1142b3ec0642efe2a6fbd38bc4714bc.png" width="416" height="150" class="jop-noMdConv">**
    ...
    - when we shutdown g0/0- 12.2 route will be added to RT
        ...
        - <img src="../../_resources/3b00ae5b6956ec1a655abd19149ef3a1.png" alt="3b00ae5b6956ec1a655abd19149ef3a1.png" width="600" height="93" class="jop-noMdConv"></img>
            - ==\[AD/metric\]- \[10/0\]==
5.  **Host route**
    ...
    - <img src="../../_resources/fb5ce4d2cc94819da486f5891b2a43ad.png" alt="fb5ce4d2cc94819da486f5891b2a43ad.png" width="404" height="249" class="jop-noMdConv">
    ...
    - `ip route 20.2.2.100 255.255.255.255 g0/2 192.168.23.2` - host route (on R1)
        ...
        - `sh ip route static`\- <img src="../../_resources/0b2ee1bddaa6a26a8ee07212c7031d30.png" alt="0b2ee1bddaa6a26a8ee07212c7031d30.png" width="554" height="54" class="jop-noMdConv">
            - R1 prefers 23.2 over 12.2 for 2.100- choose /32 over /24
    - <img src="../../_resources/5a6f8bbea5f650a29d122a8a24312324.png" alt="5a6f8bbea5f650a29d122a8a24312324.png" width="278" height="154" class="jop-noMdConv">
...
6.  **Null Route**
    ...
    - <img src="../../_resources/0350e5d8258539de26aadfb0ea46f434.png" alt="0350e5d8258539de26aadfb0ea46f434.png" width="344" height="164" class="jop-noMdConv">
