...
- ### **<ins>BPDU types</ins>**\- BPDU used to identify a hierarchy & notify changes in topology
    ...
    - **Config** **BPDU(35B)**\- used to identify Root Bridge, Sender Port ID/Port identifier (root/designated/blocked ports), ...
        ...
        - ![861c5b7e2ceb176af9e6db35b12fe92f.png](../../_resources/861c5b7e2ceb176af9e6db35b12fe92f.png)
    ...
    - **TCN BPDU** (Topology change Notification)**(4B)-** inform other switches in net about **L2 topology changes** (switch failure, link failure due to port, new switch added/removed to network)
        ...
        - <img src="../../_resources/2ec0bc351e182cd0cf52c16e60302728.png" alt="2ec0bc351e182cd0cf52c16e60302728.png" width="253" height="111" class="jop-noMdConv">\- length of octet in bytes
...
- <img src="../../_resources/d970a477ed18cd59ac5bdb88e4398cdd.png" alt="d970a477ed18cd59ac5bdb88e4398cdd.png" width="457" height="288" class="jop-noMdConv"></img>
...
- <img src="../../_resources/534f085a7d8aa9712ddd08af6d2d05ea.png" alt="534f085a7d8aa9712ddd08af6d2d05ea.png" width="452" height="303" class="jop-noMdConv"> - 52s convergence for SW2 (15s-step6 + 15s+20s- step5 + 2s-BPDU)

### Indirect Link Failures- comm btw SW is filtered, link up

- <img src="../../_resources/1188890ec90b05d90bc1cf11a8cc3cd9.png" alt="1188890ec90b05d90bc1cf11a8cc3cd9.png" width="443" height="292" class="jop-noMdConv">\- 52s convergence for SW3
...
- ### Port Roles
    ...
    - **<ins>AP (Alternate Port)</ins>**\- port provides alternate path toward RB, remains in ==Blocking state==, ready to take over if RP fails. Ports other than RP & DP are AP
...
- ## <ins>**STP Working**</ins>
    ...
    2.  ### **Path Selection:**
        ...
        - <img src="../../_resources/2e409e86cf9e15b532e95de43e8565e5.png" alt="2e409e86cf9e15b532e95de43e8565e5.png" width="510" height="240" class="jop-noMdConv">
    3.  ### **RP Election**
        ...
        - <ins>Lowest Sender Port ID</ins> (eg - g0/0, g0/1)
            ...
            - <img src="../../_resources/64bf4f690904e7cd4aa922d7f5af2cca.png" alt="64bf4f690904e7cd4aa922d7f5af2cca.png" width="310" height="77" class="jop-noMdConv">\- Sw5 g0/1=RP as opp side(Sw4) is g0/0 i.e. lowest
        - <ins>Lowest Receiver Port ID</ins>
            
            - <img src="../../_resources/fd8dccbe7ea0032c6768f2cb255ca627.png" alt="fd8dccbe7ea0032c6768f2cb255ca627.png" width="245" height="75" class="jop-noMdConv">\- Sw5 g0/0=RP as its lowest than Sw5 g0/1
    4.  ### **DP Election**
...
- ## STP Protocols
    ...
    - **STP (Spanning Tree Protocol)-** IEEE 802.1D, takes 50s(20+15+15) for ports to come up, 1 STP(1 RB) for all VLANs
    ...
    - **<span style="color: rgb(35, 111, 161);">PVST+ (Per VLAN Spanning Tree Plus)(by default running in cisco)</span>\-** 50s, separate ST instance for each VLAN, 1 STP per VLAN(CPU overhead) (in cisco sh command - ==<ins>ieee</ins>\= pvst+==)
        
        - ![7a2944e7ac27a4a0720ff2349d849e04.png](../../_resources/7a2944e7ac27a4a0720ff2349d849e04.png)
    ...
    - **Practical**
        ...
        - <img src="../../_resources/cf9e2c58f0876f9445fd0f648eb07750.png" alt="cf9e2c58f0876f9445fd0f648eb07750.png" width="529" height="261" class="jop-noMdConv"></img>
...
- ### Modify STP Root Port & Blocked Port
    ...
    - Changing **<ins>Port Priority</ins>**\- `int <int>`, `spanning valn 1 port-priority <0-240>` (==default PP- 128==) (increment/multiple of 16)
        - <img src="../../_resources/733871e8b31fc87723dec4a81dfea63b.png" alt="733871e8b31fc87723dec4a81dfea63b.png" width="434" height="69" class="jop-noMdConv">
...
- <ins>Short</ins>
    ...
    - BPDU Filter(acess port edge-devices)- Blocks outgoing BPDU. ignore incoming BPDU. not reveal RB
...
- ### BPDU Filter (not sure- to remake)
    
    - Blocks (outgoing)BPDUs transmitted out of port. (used to not reveal RB info)
        
    - `spanning-tree portfast edge bpdufilter default`\- globally/ all int (advice- it should be done only at start of topology creation, not on running STP- hackers)
        
        - port sends 11 BPDUs. switch checks to identify preferred Sw. preferred Sw doesn’t process BPDUs but forwards them to inferior Sw. non-preferred Sw processes received BPDUs but doesn’t forward them to superior switches.
...
- ### <ins>Problems with Unidirectional Links</ins>
    ...
    - ### Unidirectional Link Detection(UDLD)
        ...
        - 2 modes
            ...
            - In normal mode, if the link state of the port was determined to be bi-directional and the UDLD information times out, no action is taken by UDLD. The port state for UDLD is marked as undetermined . The port behaves in accordance with its STP state.
...
- ## RSTP Working
    ...
    2.  Both SWs establish <ins>handshake</ins> with each other to <ins>advertise proposal</ins> (in config BPDU) that their int should be DP
        - <img src="../../_resources/1f06ea2f455d6ae0437138c841ccfa99.png" alt="1f06ea2f455d6ae0437138c841ccfa99.png" width="425" height="376" class="jop-noMdConv">
    ...
    5.  Inferior SW <ins>sends an agreement</ins> (config BPDU) to RB via RP(signifies RB, syn is occurring on that SW)
        - <img src="../../_resources/596859db9cba21ba883a3452b78a6604.png" alt="596859db9cba21ba883a3452b78a6604.png" width="445" height="302" class="jop-noMdConv">\- after all BPDU from RB to NRB
...
- <img src="../../_resources/75911a93a2406860e5fe6dfcae8438c0.png" alt="75911a93a2406860e5fe6dfcae8438c0.png" width="472" height="366" class="jop-noMdConv">
...
- IEEE 802.1S. MST protocol maps 1/multiple VLANs into 1 STP tree called MST instance
    ...
    - ![5184a6baee15ad8651f2fca3f9599c9b.png](../../_resources/5184a6baee15ad8651f2fca3f9599c9b.png)\- SW maintaining 3 STP topologies
- ### MST Region
    ...
    - <img src="../../_resources/e7ba8c7bc43f9518ba1afee47733277f.png" alt="e7ba8c7bc43f9518ba1afee47733277f.png" width="323" height="152" class="jop-noMdConv">
- ### MST Region Criteria
...
- <img src="../../_resources/df99611244684af8d5c24f24d2e3f3c7.png" alt="df99611244684af8d5c24f24d2e3f3c7.png" width="409" height="181" class="jop-noMdConv"></img>
    - ```bash
        int r g0/2-3, g1/0-3
        sh
        hostname SW-2
